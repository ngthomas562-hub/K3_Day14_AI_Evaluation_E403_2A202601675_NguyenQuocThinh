...
# A metric average that drops by more than this vs baseline counts as a regression.
...
def _mean(values: list[float]) -> float:
    """Average that returns 0.0 instead of raising on an empty list."""
...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        # Step 1 — mark each chunk relevant/not, keeping the retriever's order.
        relevance_flags = [
        ...
        # Steps 2-3 — Precision@k summed over the relevant ranks only, so a
        # relevant chunk sitting at rank 1 contributes more than at rank 5.
        hits = 0
        precision_sum = 0.0
    ...
    def run_full_eval(
        ...
        # Retrieval metrics are diagnostic only: they stay None without a
        # retrieval trace and never feed passed/overall_score().
        context_recall: float | None = None
        context_precision: float | None = None
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    # original order and the returned set is exactly the input set.
    return sorted(
        contexts,
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "criterion using a number from 0.0 to 1.0.\n"
            "Judge only whether the content satisfies the criterion; do not "
            "reward an answer for being longer.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{criteria}\n\n"
    ...
    def _parse_scores(raw: str, rubric: dict[str, Any]) -> dict[str, float]:
        """Pull one score per criterion out of the judge reply.

        Anything unparseable falls back to a neutral 0.5 so a malformed judge
        response degrades one criterion instead of crashing the benchmark.
        """
        parsed: dict[str, Any] = {}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        entry_means = [
            ...
            if (values := (entry.get("scores") or {}).values())
        ]
        if not entry_means:
            return {
        ...
        overall = _mean(entry_means)
        # Position bias shows up as the first-scored response beating the rest
        # by a visible margin; a single entry proves nothing either way.
        positional_bias = (
            len(entry_means) >= 2
            and entry_means[0] - _mean(entry_means[1:]) > 0.1
        )
        return {
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Keep the caller's pair so metadata (id, difficulty) survives the
            # round trip for reporting and failure analysis.
            result.qa_pair = pair
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        def optional_avg(attribute: str) -> float | None:
            scored = [
                value
                for result in results
        ...
        return {
            ...
            "avg_context_recall": optional_avg("context_recall"),
            "avg_context_precision": optional_avg("context_precision"),
...
class FailureAnalyzer:
    ...
    # One targeted fix per failure category, so clustered failures map to a
    # single action instead of a per-answer patch.
    SUGGESTION_BY_TYPE: dict[str, str] = {
        "hallucination": (
            "Implement a hallucination checker that drops claims absent from "
            "the retrieved context before the answer is returned"
        ),
        "irrelevant": (
            "Tighten the prompt so the model restates and addresses every part "
            "of the question before answering"
        ),
        "incomplete": (
            "Raise retrieval top-k and add few-shot examples showing answers "
            "that keep every date, amount, condition and exception"
        ),
        "off_topic": (
            "Add intent routing so out-of-domain questions are detected before "
            "generation instead of being answered from the wrong document"
        ),
        "refusal": (
            "Relax the guardrail so the assistant answers when the retrieved "
            "context does support a reply"
        ),
    }

    GENERAL_SUGGESTIONS: tuple[str, ...] = (
        "Rerank retrieved chunks so evidence-bearing passages lead the context "
        "window",
        "Split long corpus paragraphs into smaller chunks to cut retrieval "
        "noise",
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # Two or more weak metrics mean no single stage explains the failure.
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            # suggestions may be shorter than failures; reuse the last one so
            # every failure still logs an action.
            if index < len(suggestions):
                fix = suggestions[index]
            elif suggestions:
                fix = suggestions[-1]
            else:
                fix = "Review the retrieval and generation trace"
            rows.append(
                f"| F{index + 1:03d} "
    ...
    def generate_improvement_suggestions(
        ...
        # Most frequent failure category first — fixing it clears the most cases.
        for failure_type, _count in sorted(
            categories.items(), key=lambda item: (-item[1], item[0])
        ):
            suggestion = self.SUGGESTION_BY_TYPE.get(failure_type.lower())
